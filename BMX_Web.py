...
st.markdown("""
<style>
/* Apagar por completola barra superior flotante original de Streamlit Cloud */
...
@media print {
    /* Escondemos de inmediato los controles de la interfaz y la barra lateral */
    .viewerBadge, footer, button, .stChatInput, [data-testid="stSidebar"], div[class*="stBottom"], .stHorizontalBlock {
        visibility: hidden !important;
        display: none !important;
    }
    /* Forzamos que el PDF tome un fondo blanco limpio para que se lea perfectamente */
    html, body, .stApp {
    ...
    with st.sidebar:
        ...
        if not st.session_state.suscrito:
            ...
            if st.button(L["cerrar_sesion"]):
                ...
                st.rerun()
# --- ÁREA CENTRAL ---
# Fila horizontal superior con los controles independientes libres de gadgets
col_control_1, col_control_2 = st.columns()

with col_control_1:
    texto_boton_sidebar = "➡️ Mostrar Menú" if not st.session_state.sidebar_visible else "⬅️ Esconder Menú"
    if st.button(texto_boton_sidebar, use_container_width=True):
        st.session_state.sidebar_visible = not st.session_state.sidebar_visible
        st.rerun()

with col_control_2:
    # SE AGREGA WINDOW.PARENT.PRINT: Forzado estructural para capturar la app entera
    st.markdown(
        ...
        'font-size: 14px; width: 100%; cursor: pointer;">🖨️ Imprimir Certificado</button>'
        '</div>',
        unsafe_allow_html=True
    )

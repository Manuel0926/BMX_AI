...
/* 2. REMOCIÓN TOTAL POR CSS DE LOS ICONOS INFERIORES DERECHOS */
footer, 
...
# --- INYECTOR DE SCRIPT JAVASCRIPT AVANZADO (MUTATION OBSERVER DEFINITIVO) ---
st.components.v1.html("""
<script>
const eliminarControlesNativos = () => {
    // 1. Apuntar a la barra de administración inferior (Corona de Streamlit Cloud)
    const badge = window.parent.document.querySelector('.viewerBadge') || window.parent.document.querySelector('iframe[title="managed-hosted-app-badge"]');
    if (badge) {
        badge.remove();
    }

    // 2. Apuntar al contenedor general de estado y menú flotante derecho
    const toolbar = window.parent.document.querySelector('[data-testid="stToolbar"]') || window.parent.document.querySelector('[data-testid="manage_app_button"]');
    if (toolbar) {
        toolbar.remove();
    }

    // 3. Eliminar bloques fijos inyectados abajo por el servicio de hosting
    const fijos = window.parent.document.querySelectorAll('div[style*="position: fixed"][style*="bottom"]');
    fijos.forEach(el => {
        if (!el.querySelector('input') && !el.querySelector('textarea')) { // No borrar el chat
            el.remove();
        }
    });
};
...
// Ejecución inmediata
eliminarControlesNativos();

// Observador en tiempo real para evitar que reaparezcan tras interacciones del usuario

...
# --- CONTROL CSS COMPLETO: OCULTA GADGETS Y REPARA EL BOTÓN DE APERTURA DEL SIDEBAR ---
st.markdown("""
<style>
/* 1. Ocultar la línea decorativa superior y el menú de tres puntos original de raíz */
[data-testid="stDecoration"], button#MainMenu, #MainMenu {
    visibility: hidden !important;
    display: none !important;
}

/* 2. Ocultar los botones de desarrollo (Fork e icono de GitHub) inyectados en la cabecera */
.stActionButton, .stMainActionButton, [data-testid="stActionButton"], .stAppHeader button:not([data-testid="stHeaderActionButton"]) {
    visibility: hidden !important;
    display: none !important;
}

/* 3. CORRECCIÓN CRÍTICA: Hacer visible el encabezado pero transparente para que no tape el botón '>' */
...
/* Forzar que el fondo azul se aplique al cuerpo de la app sin tapar los botones de control */
.stApp {
    background-color: #7CB4E6 !important;
}

/* Asegurar que el botón de expandir/contraer el sidebar y el de imprimir permanezcan visibles y clicables */
button[data-testid="stHeaderActionButton"], [data-testid="stHeaderActionButton"], button[data-testid="stSidebarCollapseButton"] {
    visibility: visible !important;
    display: inline-flex !important;
    z-index: 999999 !important;
}

/* 4. Ocultar por completo la barra flotante inferior 'Manage app' y el pie de página de Streamlit */
...
with st.sidebar:
    ...
    if not st.session_state.suscrito:
        ...
        # EXTRACCIÓN REAL DE VECTOR STORES CONECTADOS A TU CONSOLA DE GOOGLE
        try:
            ...
            if opciones:
                ...
                st.warning("⚠️ No se encontraron directorios (Stores) creados en tu consola de Google.")
            ...
            try:
                ...
                with st.chat_message("assistant"):
                    with st.spinner(L["spinner_vector"]):
                        ...
                        # CONEXIÓN INTEGRADA CON GEMINI 2.5 FLASH BAJO LA SINTAXIS v1beta

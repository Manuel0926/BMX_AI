...
# --- INTERFAZ PREMIUM TOTALMENTE DEPURADA ---
st.markdown("""
<style>
header, [data-testid="stHeader"], .stAppHeader, .stApp, [data-testid="stSidebar"], [data-testid="stSidebar"] > div:first-child, [data-testid="stChatInputHoverContainer"], div[data-testid="stBottom"] > div {
...
@media print {
html, body, .stApp, header, [data-testid="stHeader"], .stAppHeader, [data-testid="stSidebar"], [data-testid="stSidebar"] > div:first-child, [data-testid="stChatInputHoverContainer"], div[data-testid="stBottom"] > div, div[data-testid="stExpander"], .sidebar-card, button, div[data-testid="stLinkButton"] a { background: #FFFFFF !important; background-color: #FFFFFF !important; box-shadow: none !important; color: #000000 !important; }
...
class BMXWebScanner:
    def __init__(self):
        # Se remueve la clave escrita en texto plano para evitar bloqueos automatizados de Google
    ...
    def obtener_cliente(self):
        # EXTRACCIÓN SEGURA REQUERIDA: Lee dinámicamente tu clave desde la configuración secreta de Streamlit
        if "GEMINI_API_KEY" in st.secrets:
            api_key_segura = st.secrets["GEMINI_API_KEY"]
        else:
            # Fallback en caso de que utilices una variable de entorno local en CMD/Terminal
            api_key_segura = os.getenv("GEMINI_API_KEY", "")

        if not api_key_segura:
            raise ValueError(
                "No se detectó 'GEMINI_API_KEY' en los Secrets de Streamlit ni en las variables de entorno.")
...
with st.sidebar:
    ...
    # CARGA RECTIFICADA DEL GIF: Vinculado desde la URL del respositorio público para garantizar ejecución en la nube
    # --- RENDERIZADO DEL GIF COMPATIBLE CON STREAMLIT CLOUD (URL RAW DIRECTA) ---
    # --- RENDERIZADO DEL GIF COMPATIBLE CON STREAMLIT CLOUD (URL RAW CON MAYÚSCULA CORRECTA) ---
#    URL_GIF_RAW = "https://githubusercontent.com"

 #   st.markdown(
 #       f'<img src="{URL_GIF_RAW}" style="width:100%; border-radius:8px; margin-bottom:15px;" alt="BMX.AI Animation">',
 #       unsafe_allow_html=True
 #   )

 #   st.markdown("---")

    # --- RENDERIZADO DEL GIF MEDIANTE CONVERSIÓN DE BYTES LOCALES ---
    import base64
    ...
    if not st.session_state.suscrito:
        ...
        # EXTRACCIÓN DE ALMACENES VECTORIALES CON CAPTURA DE ERROR EN TIEMPO REAL
        try:
            ...
            if opciones:
                ...
                st.warning("⚠️ No se encontraron directorios (Stores) creados en tu proyecto de Gemini.")

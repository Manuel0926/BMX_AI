...
st.markdown("""
<style>
/* 1. Ocultar bloque superior y controles nativos flotantes */
...
/* 2. OCULTACIÓN TOTAL DE ICONOS INFERIORES POR SELECTOR ANIDADO */
footer, 
...
.viewerBadge,
.stAppDeployButton,
div[class*="stBottom"] > div:nth-child(2) { 
...
/* Regla especial para romper los contenedores globales de la barra de herramientas de Streamlit */
div[data-testid="stToolbar"], div[class*="stMainMenu"] {
 display: none !important;
 visibility: hidden !important;
}

.stApp { background-color: #7CB4E6 !important; margin-top: 0px !important; }
...
[data-testid="stChatInputHoverContainer"], div[data-testid="stBottom"] > div { background-color: #7CB4E6 !important; }
...
st.components.v1.html(
    "<script>const stop = () => { window.parent.document.querySelectorAll('input').forEach(el => { el.setAttribute('autocomplete', 'off'); el.setAttribute('name', Math.random().toString(36).substring(7)); }); }; stop(); setInterval(stop, 500);</script>",
    height=0, width=0)
...
class BMXWebScanner:
    ...
    def _inicializar_db(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            '''CREATE TABLE IF NOT EXISTS usuarios (id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT, email TEXT UNIQUE, plan_suscripcion TEXT, fecha_pago TEXT, activo INTEGER DEFAULT 0, contador_ingresos INTEGER DEFAULT 0, fecha_registro TEXT)''')
        cursor.execute("PRAGMA table_info(usuarios)")
        columnas = [col[1] for col in cursor.fetchall()]
        if "contador_ingresos" not in columnas: cursor.execute(
            "ALTER TABLE usuarios ADD COLUMN contador_ingresos INTEGER DEFAULT 0")
    ...
    # CÓDIGO CORREGIDO
    def obtener_cliente(self):
        # 1. Intentar leer primero la Variable de Entorno (Prioridad para Hugging Face Docker)
        api_key_segura = os.environ.get("GEMINI_API_KEY", "")
        # 2. Si está vacío, intentar buscar en st.secrets (Por si pruebas localmente en el futuro)
        if not api_key_segura:
            try:
                api_key_segura = st.secrets["GEMINI_API_KEY"]
            except Exception:
                pass  # Si falla o no existe, continúa sin romper la app
        # 3. Validar de forma definitiva
        if not api_key_segura:
            raise ValueError("No se detectó 'GEMINI_API_KEY' ni en las Variables de Entorno ni en Secrets.")
        return genai.Client(api_key=api_key_segura, http_options={'api_version': 'v1beta'})
...
LOCALES = {"es": {"marca": "BMX Certificado IA", "ya_reg": "🔑 Ya estoy registrado", "mail_reg": "Email registrado:",
                  "btn_val": "Validar",
                  "err_mail": "❌ El formato del correo electrónico no es válido. Ej: usuario@dominio.com",
                  "act_pend": "Usuario registrado, pero pendiente de activación.",
                  "no_encontrado": "Email no encontrado.", "nuevo_reg": "📝 Nuevo Registro",
                  "nom_com": "Nombre completo:", "corr_elec": "Correo electrónico:", "plan_des": "Plan deseado:",
                  "btn_reg": "Registrar", "exito_reg": "✅ Registro exitoso. ¡Puedes entrar sin pago!",
...
if st.session_state.sidebar_visible:
    with st.sidebar:
        ...
        # --- NUEVO BLOQUE DE DETECCIÓN INTELIGENTE PARA EL GIF ---
        ruta_src = os.path.join(os.path.dirname(__file__), "Mongoose.gif")
        ruta_raiz = os.path.join(os.getcwd(), "Mongoose.gif")
        # El sistema busca el archivo automáticamente en ambas ubicaciones
        ...
        if ruta_final:
            try:
                ...
                st.sidebar.error(f"Error de renderizado: {e}")
        else:
            # Si el archivo de verdad no existe en ningún lado, esto te avisará en pantalla
            st.sidebar.warning("Aviso: Mongoose.gif no se encuentra en el repositorio.")
        st.markdown("---")

    ...
    with st.sidebar:
        ...
        if not st.session_state.suscrito:
            ...
            if st.button(L["cerrar_sesion"]):
                ...
                st.rerun()
# --- ÁREA CENTRAL ---
# Fila horizontal superior con los controles personalizados de Impresión y Despliegue de Menú Lateral
col_control_1, col_control_2 = st.columns([8, 2])

with col_control_1:
    # Botón dinámico personalizado para alternar de forma segura el menú lateral
...
with col_control_2:
    # Botón de impresión inyectado con HTML nativo de alta velocidad
    st.markdown(
        ...
        'font-size: 14px; cursor: pointer;">🖨️ Imprimir</button>'
        '</div>',
        unsafe_allow_html=True
    )

...
class BMXWebScanner:
    def __init__(self):
        self.API_KEY_PREDEFINIDA = "AIzaSyAkR020QSAUy4xBLuY3d2nAHuZL2vfkJ40"
    ...
    def obtener_cliente(self):
        # Se añade api_version obligatorio de la beta para interactuar con archivos de almacenes de Vertex
        return genai.Client(api_key=self.API_KEY_PREDEFINIDA, http_options={'api_version': 'v1beta'})
...
# CONTROL DE ESTADO DE APERTURA NATIVO PARA EXPANDERS
...
# IDENTIFICADORES DINÁMICOS PARA FORZAR LA CONTRACCIÓN ABSOLUTA TRAS ACCIÓN CORRECTA
...
LOCALES = {
    "es": {
        ...
        "err_store": "Error conectando con Stores.",
...
with st.sidebar:
    ...
    # RENDERIZADO MEJORADO CON BASE64 PARA EVITAR CONGELAMIENTO EN LA NUBE
    if os.path.exists("mongoose.gif"):
        with open("mongoose.gif", "rb") as gif_file:
            gif_bytes = gif_file.read()
            gif_b64 = base64.b64encode(gif_bytes).decode("utf-8")
        st.markdown(
            f'<img src="data:image/gif;base64,{gif_b64}" style="width:100%; border-radius:8px; margin-bottom:15px;">',
            unsafe_allow_html=True
        )
    ...
    if not st.session_state.suscrito:
        ...
        # OBTENCIÓN CORREGIDA DE LOS VECTOR STORES MEDIANTE LLAMADA ESTABLE AL SUBMÓDULO DE LA CLASE
    ...
    if "current_store" in st.session_state:
        ...
        if prompt := st.chat_input(L["chat_input"]):
            ...
            with st.chat_message("user"):
                st.markdown(prompt)
            try:
                # LLAMADA AL CLIENTE INSTANCIADO CON LA VERSIÓN BETA REQUERIDA PARA OPERACIONES VECTORIALES
                client = app.obtener_cliente()

...
# --- REGLAS CSS TOTALES MANTENIENDO EL DISEÑO COMPACTO ---
st.markdown("""
<style>
/* 1. Apagar por completo la barra superior original de Streamlit Cloud */
...
/* 2. RECTIFICACIÓN CORPORATIVA: Color de fondo azul en el cuerpo general */
.stApp {
    background-color: #7CB4E6 !important;
}

/* Margen superior e interno seguro para optimizar el campo visual */
.block-container {
...
/* Ajustar el espaciado entre bloques nativos de Streamlit */
[data-testid="stVerticalBlock"] {
    gap: 0.5rem !important;
}

/* 3. Ocultar la barra inferior 'Manage app' y el pie de página de la nube */
...
[data-testid="stStatusWidget"], .stStatusWidget, div[class*="stConnectionStatus"], div[class*="stAppHostButton"], button[class*="stAppHostButton"] {
    visibility: hidden !important;
    display: none !important;
...
/* Reducción del 10% en la barra lateral */
[data-testid="stSidebar"] *, [data-testid="stSidebar"] p, [data-testid="stSidebar"] span, [data-testid="stSidebar"] label, [data-testid="stSidebar"] input, [data-testid="stSidebar"] select {
...
.chat-header-title { 
    font-family: sans-serif !important; color: #000000 !important; font-weight: 700 !important; font-size: 1.9rem !important; border-bottom: 3px solid #000000 !important; margin-bottom: 15px; padding-bottom: 6px; display: inline-block; 
}

.sidebar-brand-title { 
    font-family: sans-serif !important; color: #000000 !important; font-weight: 700 !important; font-size: 1.17rem !important; letter-spacing: -0.5px !important; text-align: center; margin-bottom: 15px; white-space: nowrap !important; 
}
div[data-testid="stExpander"] {
    background-color: #639FD3 !important; border: 1px solid #000000 !important; border-radius: 10px !important; 
}
.sidebar-card { 
    background-color: #4B8BC2 !important; border: 1px solid #000000 !important; padding: 12px !important; border-radius: 10px !important; margin-bottom: 12px !important; 
}
...
# Estado nativo para controlar la sincronización del botón
...
LOCALES = {
    "es": {
        ...
        "bienvenida_html": """
<div style="font-family: sans-serif; line-height: 1.4; background-color: #7CB4E6; padding: 5px; margin: 0 auto; color: #000000; max-width: 1100px; width: 100%;">
<div style="text-align: center; margin-bottom: 20px;">
    <h2 style="color: #000000; font-weight: 800; font-size: 1.95rem; margin: 0 0 6px 0; font-family: sans-serif;">Consulta BMX Old School</h2>
    <div style="width: 260px; height: 3px; background-color: #000000; margin: 0 auto 15px auto;"></div>
    <h3 style="color: #000000; font-weight: 800; font-size: 1.35rem; margin: 0; font-family: sans-serif;">¡Bienvenidos a la fuente oficial del BMX Old School!</h3>
</div>
<table style="width: 100%; border-collapse: collapse; border: none; margin-top: 5px;">
<tr>
<td style="width: 53%; vertical-align: top; padding-right: 30px; border-right: 2px solid #000000; text-align: justify;">
<p style="font-size: 0.92rem; margin-top: 0; margin-bottom: 14px; color: #000000; line-height: 1.45;">Este es el espacio donde nuestros recuerdos hoy se convierten en <strong>pasión</strong>. Gracias al poder de la <strong>Inteligencia Artificial (modelo RAG)</strong>, dejamos atrás los mitos y las leyendas urbanas.</p>
<p style="font-size: 0.92rem; margin-bottom: 0; color: #000000; line-height: 1.45;">Ahora tienes el power de interrogar directamente a las <strong>fuentes oficiales de la época</strong>: revistas, catálogos de bicicletas y repuestos, pruebas de rendimiento de fábrica y mucho más. Todo en un solo lugar para que <strong>certifiques tu conocimiento</strong> con datos reales.</p>
</td>
<td style="width: 47%; vertical-align: top; padding-left: 30px; text-align: left;">
<h4 style="margin-top: 0; color: #000000; font-weight: 800; font-size: 1.1rem; margin-bottom: 10px; font-family: sans-serif;">¡Inicia ahora!</h4>
<p style="margin: 4px 0; font-size: 0.9rem; color: #000000; line-height: 1.4;">1. Regístrate y valida tu acceso con tu correo electrónico en la barra lateral.</p>
<p style="margin: 4px 0; font-size: 0.9rem; color: #000000; line-height: 1.4;">2. Realiza tus consultas y exporta los resultados en un documento PDF listo para imprimir.</p>
<p style="font-style: italic; font-size: 0.85rem; margin-top: 15px; margin-bottom: 10px; color: #000000; line-height: 1.35;">Esta herramienta está hecha por y para la comunidad. Contamos contigo para evolucionar, aprender, corregir y mejorar este proyecto juntos.</p>
<p style="margin-top: 15px; margin-bottom: 0; font-weight: bold; color: #000000; font-size: 0.9rem; font-family: sans-serif;">Atentamente,<br><br>Manuel Sanabria</p>
</td>
</tr>
</table>
</div>
...
# Se procesa el sidebar nativo de Streamlit
with st.sidebar:
            ...
            if st.button(L["btn_reg"]):
                ...
                if not es_correo_valido(correo_limpio):
                    ...
                    try:
                        ...
                        st.warning("⚠️ Este correo ya se encuentra registrado en el sistema.")
                    except Exception as error_externo:
                        st.error(f"Fallo técnico: {error_externo}")
...
with col_control:
    # 1. Determinamos dinámicamente el texto según la memoria de sesión
    texto_boton_sidebar = ">>" if not st.session_state.sidebar_visible else "<<"

    # 2. Al dar clic, cambiamos el interruptor en memoria e inyectamos la orden en el árbol del DOM principal
    if st.button(texto_boton_sidebar, use_container_width=False, key="btn_toggle_sidebar_master"):
        st.session_state.sidebar_visible = not st.session_state.sidebar_visible

        # INYECCIÓN MAESTRA: Buscamos el control original oculto del sidebar y disparamos su evento click nativo
        st.components.v1.html("""
        <script>
        const barra = window.parent.document.querySelector('button[data-testid="stSidebarCollapseButton"]');

...
# --- CSS COMPATIBLE CON LA NUEVA ACTUALIZACIÓN DE STREAMLIT ---
st.markdown("""
<style>
/* 1. Ocultar el menú de tres puntos, el botón de Deploy y la decoración superior */
header, [data-testid="stHeader"], .stAppHeader, button[data-testid="stHeaderActionButton"], [data-testid="stDecoration"], .stDeployButton {
    visibility: hidden !important;
    display: none !important;
}

/* Forzar que el botón nativo de Share permanezca visible si se requiere imprimir */
header button:has(svg.bi-share), header button[title*="Share"] {
    visibility: visible !important;
    display: inline-flex !important;
}

/* 2. Ocultar de raíz la barra flotante inferior 'Manage app' para los clientes */
footer, [data-testid="manage_app_button"], button:has(div:contains("Manage app")), div[class*="stBottom"] > div:nth-child(2) {
    visibility: hidden !important;
    display: none !important;
}

/* 3. Bloquear insignias o badges flotantes del hosting de Streamlit Cloud */
...
class BMXWebScanner:
    ...
    def configurar_api(self):
        # Conexión ultra estable usando variables de entorno nativas de Streamlit Secrets
...
if "expander_registro" not in st.session_state: st.session_state.registro_id = 0
LOCALES = {
    "es": {
...
with st.sidebar:
    ...
    # RENDERIZADO DEL GIF USANDO BYTES DIRECTOS DESDE LA RAÍZ DEL SERVIDOR LINUX
    try:
    ...
    if not st.session_state.suscrito:
        ...
        # EXTRACCIÓN MEDIANTE MÉTODOS HEREDADOS ESTABLES (A PRUEBA DE CAMBIOS DE COMPILADORES DE STREAMLIT)
        try:
            app.configurar_api()
            # Simulación o enlace directo con los almacenes vectoriales de largo contexto
            ...
            try:
                ...
                with st.chat_message("assistant"):
                    with st.spinner(L["spinner_vector"]):
                        # Uso de la API Legacy ultra-estable y compatible con todas las versiones de Python en la nube

...
if not st.session_state.suscrito:
    # 1. TÍTULO PRINCIPAL CON MARGEN SUPERIOR SEGURO
    st.markdown(
        '<h2 style="text-align: center; color: #000000; font-weight: 900; font-size: 2.2rem; margin-bottom: 35px;">¡Bienvenidos a la fuente oficial del BMX Old School!</h2>',
        unsafe_allow_html=True)

    # 2. BLOQUE DE TEXTO REESTRUCTURADO CON COLUMNAS NATIVAS DE STREAMLIT (Adiós a los recortes HTML)
    col_bienvenida_1, col_bienvenida_2 = st.columns(2)

    with col_bienvenida_1:
        st.markdown("""
        Este es el espacio donde nuestros recuerdos hoy se convierten en **pasión**. 
        Gracias al poder de la **Inteligencia Artificial (modelo RAG)**, dejamos atrás los mitos y las leyendas urbanas.

        Ahora tienes el power de interrogar directamente los **recursos históricos oficiales**: revistas, catálogos de bicicletas y repuestos, pruebas de rendimiento de fábrica y mucho más. Todo en un solo lugar para que puedas **certificar tu conocimiento** con datos factuales.
        """)

    with col_bienvenida_2:
        st.markdown("### ¡Inicia ahora!")
        st.markdown("1. **Regístrate y valida tu acceso** con tu correo electrónico en la barra lateral.")
        st.markdown("2. Realiza tus consultas de forma inmediata.")
        st.markdown("""
        *Esta herramienta está hecha por y para la comunidad. Contamos contigo para evaluar, aprender, corregir y mejorar este proyecto juntos.*

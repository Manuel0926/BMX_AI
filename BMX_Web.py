...
# --- REGLAS CSS MEJORADAS PARA PERMITIR LA INTERACCIÓN JAVASCRIPT ---
st.markdown("""
<style>
/* 1. Ocultar la barra superior sin destruir sus botones internos del árbol HTML */
...
/* RECTIFICACIÓN CRÍTICA: Ajuste de posición superior para compensar la cabecera oculta */
.block-container {
...
/* Hacer invisible el botón de colapso original pero conservarlo activo para JavaScript */
...
with col_control:
    ...
    # Al dar clic, cambiamos el indicador en memoria y disparamos de inmediato el clic en el botón invisible de Streamlit
    if st.button(texto_boton_sidebar, use_container_width=False, key="btn_toggle_sidebar_master"):
        st.session_state.sidebar_visible = not st.session_state.sidebar_visible

        # INYECCIÓN DIRECTA SEGURA: Activamos el disparador de eventos del navegador buscando el botón preservado
        st.components.v1.html("""
        <script>
        const botonStreamlit = window.parent.document.querySelector('button[data-testid="stSidebarCollapseButton"]');
        if (botonStreamlit) { botonStreamlit.click(); }
        </script>
        """, height=0, width=0)

...
import time
import tempfile
import re  # Validación de formato de correo
...
class BMXWebScanner:
    ...
    def verificar_usuario_db(self, email):
        conn = sqlite3.connect(DB_PATH)
        # Se agregan al SELECT las columnas contador_ingresos y fecha_registro
        user = conn.execute(
...
LOCALES = {
 ...
 <h2 style="text-align: center; color: #000000; font-weight: 900; font-size: 1.6rem; margin-bottom: 20px;">
 ¡Bienvenidos a la fuente oficial del BMX Old School!
 </h2>
...
with st.sidebar:
    ...
    if os.path.exists("mongoose.gif"):
        st.image("mongoose.gif", use_container_width=True)
    st.markdown("---")
            ...
            if st.button(L["btn_val"]):
                ...
                if not es_correo_valido(correo_limpio):
                    ...
                    if resultado:
                        ...
                        if activo == 1:
                            # Se calcula el nuevo total para asignarlo de forma inmediata en esta sesión
                            nuevo_contador = (cont or 0) + 1
                            app.ejecutar_sql(
                                "UPDATE usuarios SET contador_ingresos = ? WHERE email = ?",
...
if not st.session_state.suscrito:
    ...
    # --- MODIFICACIÓN VISUAL: SE AGREGA EL CONTADOR Y LA FECHA DE REGISTRO EN LA LÍNEA DE BIENVENIDA ---
    st.write(

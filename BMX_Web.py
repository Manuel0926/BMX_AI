...
# --- ELIMINACIÓN ABSOLUTA DE LA FRANJA DE DESARROLLO DE LA NUBE ---
st.markdown("""
<style>
/* 1. Apagar por completo el bloque superior flotante de la cabecera de Streamlit Cloud */
header, [data-testid="stHeader"], .stAppHeader, div[data-testid="stHeader"], [data-testid="stDecoration"] {
...
/* Estilo para fijar el botón de impresión personalizado de tu marca */
.boton-imprimir-fijo {
    position: absolute;
    top: 20px;
    right: 30px;
...
/* Ocultar el botón automáticamente del papel físico cuando se genere el PDF */
@media print {
    .viewerBadge, footer, .boton-imprimir-fijo, button, .stChatInput, [data-testid="stSidebar"] {
        visibility: hidden !important;
        display: none !important;
    }
    html, body, .stApp {
...
with st.sidebar:
    ...
    if not st.session_state.suscrito:
        ...
        if st.button(L["cerrar_sesion"]):
            ...
            st.rerun()
# --- ÁREA PRINCIPAL ---
# Inyectar tu propio botón de impresión flotante premium libre de gadgets en la esquina superior derecha
st.markdown(
    '<div class="boton-imprimir-fijo">'
    '<button onclick="window.print()" style="'
    'background-color: #3A75A8; color: white; border: 2px solid #000000; '
    'border-radius: 24px; padding: 8px 24px; font-weight: bold; '
    'font-size: 14px; cursor: pointer;">🖨️ Imprimir Certificado</button>'
    '</div>',
    unsafe_allow_html=True
)

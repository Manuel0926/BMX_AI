...
# CONTROL DE ESTADO DE EXPANDERS EN MEMORIA (Evita cierres e hilos cruzados)
            ...
            if st.button(L["btn_reg"]):
                ...
                if not es_correo_valido(correo_limpio):
                    ...
                    try:
                        ...
                        # ALERTA DE ÉXITO INGENIERIL
                        st.success(L["exito_reg"])

                        # --- FLUJO AUTOMÁTICO DE CONTRACCIÓN ---
                        # Forzamos que el expander de registro cambie su estado a CERRADO (False) de inmediato
                        st.session_state.expander_registro = False
                        st.session_state.registro_id += 1

                        # Limpiamos los inputs para la siguiente sesión
                        for key in ["reg_nombre", "reg_email"]:
                            if key in st.session_state:
                                del st.session_state[key]

                        time.sleep(1.2)  # Pequeña pausa para que el usuario alcance a leer el recuadro verde
                        st.rerun()

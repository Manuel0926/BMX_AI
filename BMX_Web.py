...
# --- TRUCO CSS MAESTRO REFORZADO: COMPATIBLE CON TODAS LAS CAPAS EXTERNAS ---
st.markdown("""
<style>
...
/* 3. REFUERZO RADICAL ANTI-GADGETS: Elimina de raíz cualquier interacción o ventana flotante en la esquina inferior */
div[class*="stAppHostButton"], button[class*="stAppHostButton"], 
div[class*="stFeedbackButton"], button[class*="stFeedbackButton"],
[data-testid="stStatusWidget"], .stStatusWidget, div[class*="stConnectionStatus"], div[class*="ConnectionStatus"],
div[class*="stToolbar"], div[class*="AppDeployDropdown"], iframe[title*="badge"],
div:has(> button[class*="AppHostButton"]), .viewerBadge, iframe[title="managed-hosted-app-badge"],
div[class*="StyledAppPage"] + div, div:has(button[title*="profile"]), div[class*="FloatingActionButton"],
div[class*="stConnectionStatus"] + div, div[class*="stApp"] + div, body > div:not(.stApp):not([data-testid="stSidebar"]) {
    visibility: hidden !important;
    display: none !important;
    opacity: 0 !important;
    pointer-events: none !important; /* Desactiva físicamente la respuesta al mouse */
}

/* Ocultar el marco del visor e insignias de la plataforma */
.viewerBadge, iframe[title="managed-hosted-app-badge"], [class*="viewerBadge"] {
    visibility: hidden !important;
    display: none !important;
...
class BMXWebScanner:
    ...
    def configurar_api(self):
        # Conexión nativa robusta heredada compatible con Streamlit Cloud
...
if st.session_state.sidebar_visible:
    with st.sidebar:
        ...
        if not st.session_state.suscrito:
            ...
            # SELECTOR ADAPTATIVO ESTABLE PARA EL DIRECTORIO ACTIVO
            ...
            if st.button(L["cerrar_sesion"]):
                ...
                st.rerun()
# --- AREA PRINCIPAL ---
col_control, _ = st.columns(2)  # Mantener una rejilla asimétrica para el botón << / >>
            ...
            try:
                ...
                with st.chat_message("assistant"):
                    with st.spinner(L["spinner_vector"]):
                        # MODELO DE LARGO CONTEXTO ROBUSTO COMPATIBLE CON STREAMLIT CLOUD
